...
var _ = Describe("Space Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind Space")
			err := k8sClient.Get(ctx, typeNamespacedName, space)
			if err != nil && errors.IsNotFound(err) {
				resource := &githubsanjivmadhavaniov1alpha1.Space{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Space")
